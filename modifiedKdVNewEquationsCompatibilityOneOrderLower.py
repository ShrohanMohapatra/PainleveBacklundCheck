...
print(showDiffNotation(Equation1), ' = 0')
print('--------------------------------------------------------------------------')
Equation1_compatibility_test = expand(Equation1/(24*sigma**3))
print('--------------------------------------------------------------------------')
...
# first Equation2 --> Equation0, Equation1Storage, Equation3

P0 = Equation0
print('--------------------------------------------------------------------------')
print('P0 = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(P0))
print('--------------------------------------------------------------------------')
...
print('P3 = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(P3))
print('--------------------------------------------------------------------------')

P1_substituted_with_P0 = expand(P1 + 12*diff(phi,x,x)*diff(phi,x)*sigma**2*P0 + \
								3*P0*(2*sigma*diff(phi,x)+u0)*diff(P0,x) - \
								6*sigma*diff(phi,x,x)*P0*(2*sigma*diff(phi,x)+u0)
								)
print('--------------------------------------------------------------------------')
print('P1_substituted_with_P0 = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(P1_substituted_with_P0))
print('--------------------------------------------------------------------------')

P2_substituted_with_P0 = expand(P2 - (diff(phi, t)+2*sigma**2*diff(phi,x,x,x))*P0 - \
								6*sigma**2*diff(phi,x,x)*diff(P0,x) - \
								6*sigma**2*diff(phi,x)*diff(P0,x,x))
print('--------------------------------------------------------------------------')
print('P2_substituted_with_P0 = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(P2_substituted_with_P0))
print('--------------------------------------------------------------------------')

P3_substituted_with_P0 = expand(P3 + diff(P0, t) + 2*sigma**2*diff(P0, (x, 3)))
print('--------------------------------------------------------------------------')
print('P3_substituted_with_P0 = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(P3_substituted_with_P0))
print('--------------------------------------------------------------------------')

reconciliation = expand(
	2*diff(phi,x)**6*P3_substituted_with_P0 - \
	diff(phi,x)**3*diff(-diff(P1_substituted_with_P0,x)*diff(phi,x)+diff(phi,x,x)*P1_substituted_with_P0-2*diff(phi,x)**2*P2_substituted_with_P0,x) + \
	3*diff(phi,x)**2*diff(phi,x,x)*(-diff(P1_substituted_with_P0,x)*diff(phi,x)+diff(phi,x,x)*P1_substituted_with_P0-2*diff(phi,x)**2*P2_substituted_with_P0)
	)
print('--------------------------------------------------------------------------')
print('reconciliation = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(reconciliation))
print('--------------------------------------------------------------------------')

# Simplifying the expressions a little bit
...
P1_substituted_with_P0 = expand(Q1 + 12*diff(phi,x,x)*diff(phi,x)*sigma**2*Q0 + \
								3*Q0*(2*sigma*diff(phi,x)+u0)*diff(Q0,x) - \
								6*sigma*diff(phi,x,x)*Q0*(2*sigma*diff(phi,x)+u0)
								)
P2_substituted_with_P0 = expand(Q2 - (diff(phi, t)+2*sigma**2*diff(phi,x,x,x))*Q0 - \
								6*sigma**2*diff(phi,x,x)*diff(Q0,x) - \
								6*sigma**2*diff(phi,x)*diff(Q0,x,x))
P3_substituted_with_P0 = expand(Q3 + diff(Q0, t) + 2*sigma**2*diff(Q0, (x, 3)))
reconciliation = expand(
	2*diff(phi,x)**6*P3_substituted_with_P0 - \
	diff(phi,x)**3*diff(-diff(P1_substituted_with_P0,x)*diff(phi,x)+diff(phi,x,x)*P1_substituted_with_P0-2*diff(phi,x)**2*P2_substituted_with_P0,x) + \
	3*diff(phi,x)**2*diff(phi,x,x)*(-diff(P1_substituted_with_P0,x)*diff(phi,x)+diff(phi,x,x)*P1_substituted_with_P0-2*diff(phi,x)**2*P2_substituted_with_P0)
...
print('--------------------------------------------------------------------------')
print('Full version of the reconciliation = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(reconciliation))
print('--------------------------------------------------------------------------')

phi_x_solution = solve(P0,diff(phi,x))[0]
print('--------------------------------------------------------------------------')
...
phi_xx_solution = solve(P1,diff(phi,x,x))[0]
print('--------------------------------------------------------------------------')
...
u0_t_solution = solve(P3,diff(u0,t))[0]
print('--------------------------------------------------------------------------')
print('u0_t_solution = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(u0_t_solution))
print('--------------------------------------------------------------------------')

phi_xx_solution = expand(phi_xx_solution.subs(diff(phi,x),phi_x_solution))
print('--------------------------------------------------------------------------')
print('phi_xx_solution = expand(phi_xx_solution.subs(diff(phi,x),phi_x_solution)) = ')
print('--------------------------------------------------------------------------')
print(showDiffNotation(phi_xx_solution))
print('--------------------------------------------------------------------------')
