...
class TestCompatibilityPDE(unittest.TestCase):
	...
	# def test_TestCase1_Burgers(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	...
	# 			# alpha # The power balancing exponent
	# 		)
	# 	U1soln = solve(compatibility_equations[0], U1(x, t))[0]
	# 	transform_new = transform.subs(U1(x, t), U1soln)
	# 	BurgersPDE_eqn = BurgersPDE.subs(f(x, t), transform_new)
	...
	# 		compatibility_equations[k] = compatibility_equations[k].subs(U1(x, t), U1soln)
	# 		compatibility_equations[k], _ = fraction(together(compatibility_equations[k].doit()))
	# 		compatibility_equations[k] = expand(compatibility_equations[k])
	...
	# 			print('Latex version')
	# 			print('------------------------------')
	# 			print_latex(showDiffNotation(compatibility_equations[k]))
	# 			print('------------------------------')
	...
	def test_TestCase2_KdV(self):
		...
		U2soln = solve(compatibility_equations[0], U2(x, t))[0]
		transform_new = transform.subs(U2(x, t), U2soln)
		KdVPDE_eqn = KdVPDE.subs(f(x, t), transform_new)
		KdVPDE_eqn, _ = fraction(together(KdVPDE_eqn.doit()))
		KdVPDE_eqn = expand(KdVPDE_eqn)
		for k in range(len(compatibility_equations)):
			compatibility_equations[k] = compatibility_equations[k].subs(U2(x, t), U2soln)
			compatibility_equations[k], _ = fraction(together(compatibility_equations[k].doit()))
			compatibility_equations[k] = expand(compatibility_equations[k])
		compatibility_equations = [compatibility_equations[k] for k in range(1, len(compatibility_equations))]
		...
		for k in range(len(compatibility_equations)):
			...
			print_latex(showDiffNotation(compatibility_equations[k]))
			print('------------------------------')
		compatibility_checker = CompatibilitySystemPDEs(compatibility_equations, phi(x, t), 20)
		depth = compatibility_checker[0]
		status = compatibility_checker[1]
		if status:
	...
	# 	U1soln = solve(compatibility_equations[0], U1(x, t))[0]
	# 	transform_new = transform.subs(U1(x, t), U1soln)
	# 	MKdVPDE_eqn = MKdVPDE.subs(f(x, t), transform_new)
	...
	# def test_TestCase4_KdVBurgers(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	# 	print('that come from the WTC Painleve analysis of the KdV-Burgers equation ....')
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	phi, f, U2 = symbols('phi f U2', cls = Function)
	# 	KdVBurgersPDE = parse_expr('diff(f(x,t),t)+f(x,t)*diff(f(x,t),x)-alpha*diff(f(x,t),x,x)+sigma*diff(f(x,t),x,x,x)')
	# 	alpha, transform, compatibility_equations = ultimatePainleve(
	# 			KdVBurgersPDE, # Input PDE
	# 			x, # The input variable used for the spatial variable
	# 			t # The input variable used for the temporal variable
	# 			# alpha # The power balancing exponent
	# 		)
	# 	U2soln = solve(compatibility_equations[0], U2(x, t))[0]
	# 	transform_new = transform.subs(U2(x, t), U2soln)
	# 	KdVBurgersPDE_eqn = KdVBurgersPDE.subs(f(x, t), transform_new)
	# 	KdVBurgersPDE_eqn, _ = fraction(together(KdVBurgersPDE_eqn.doit()))
	# 	KdVBurgersPDE_eqn = expand(KdVBurgersPDE_eqn)
	# 	for k in range(len(compatibility_equations)):
	# 		compatibility_equations[k] = compatibility_equations[k].subs(U2(x, t), U2soln)
	# 		compatibility_equations[k], _ = fraction(together(compatibility_equations[k].doit()))
	# 		compatibility_equations[k] = expand(compatibility_equations[k])
	# 	compatibility_equations = [compatibility_equations[k] for k in range(1, len(compatibility_equations))]
	# 	compatibility_equations.insert(0, KdVBurgersPDE_eqn)
	# 	for k in range(len(compatibility_equations)):
	# 		Factors = factor_list(compatibility_equations[k])
	# 		compatibility_equations[k] = Factors[1][-1][0]
	# 		print('------------------------------')
	...
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Number of cycles of reduction in KdV-Burgers equation', depth+1)
	# 	self.assertTrue(status)

	# def test_TestCase5_PotentialKdV(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	# 	print('that come from the WTC Painleve analysis of the potential KdV equation ....')
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	phi, f, U1 = symbols('phi f U1', cls = Function)

	# 	compatibility_equations = [
	# 		#expand(phi(x, t)**4*(diff(12*sigma*Derivative(phi(x, t), x)/phi(x, t) + U1(x, t), t)+1/2*diff(12*sigma*Derivative(phi(x, t), x)/phi(x, t) + U1(x, t), x)**2 + sigma*diff(12*sigma*Derivative(phi(x, t), x)/phi(x, t) + U1(x, t), x, x, x)))
	# 		-4*sigma*Derivative(phi(x, t), x)*Derivative(phi(x, t), (x, 3)) + 3*sigma*Derivative(phi(x, t), (x, 2))**2 - Derivative(U1(x, t), x)*Derivative(phi(x, t), x)**2 - Derivative(phi(x, t), t)*Derivative(phi(x, t), x),
	# 		sigma*Derivative(phi(x, t), (x, 4)) + Derivative(U1(x, t), x)*Derivative(phi(x, t), (x, 2)) + Derivative(phi(x, t), t, x),
	# 		sigma*Derivative(U1(x, t), (x, 3)) + Derivative(U1(x, t), t) + Derivative(U1(x, t), x)**2/2
	# 	]
	...
	# 	status = compatibility_checker[1]
	# 	if status:
	# 		print('The PDE system from WTC Painleve analysis of the potential KdV equation renders compatible')
	# 	else:
	# 		print('The PDE system from WTC Painleve analysis of the potential KdV equation renders incompatible')
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	self.assertTrue(status)

	# def test_TestCase5_BBM(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	...
	# 	U2soln = solve(compatibility_equations[0], U2(x, t))[0]
	# 	transform_new = transform.subs(U2(x, t), U2soln)
	# 	BBMPDE_eqn = BBM_PDE.subs(f(x, t), transform_new)
	...
	# def test_TestCase6_KuramotoSivashinsky(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	...
	# 	print('>> len(compatibility_equations)')
	# 	print(len(compatibility_equations))
	# 	U3soln = solve(compatibility_equations[0], U3(x, t))[0]
	# 	transform_new = transform.subs(U3(x, t), U3soln)
	# 	KSPDE_eqn = KS_PDE.subs(f(x, t), transform_new)
	# 	KSPDE_eqn, _ = fraction(together(KSPDE_eqn.doit()))
	# 	KSPDE_eqn = expand(KSPDE_eqn)
	# 	for k in range(len(compatibility_equations)):
	# 		compatibility_equations[k] = compatibility_equations[k].subs(U3(x, t), U3soln)
	# 		compatibility_equations[k], _ = fraction(together(compatibility_equations[k].doit()))
	# 		compatibility_equations[k] = expand(compatibility_equations[k])
	# 	# self.assertTrue(True)
	# 	compatibility_equations = [compatibility_equations[k] for k in range(1, len(compatibility_equations))]
	...
	# def test_TestCase7_Kawachara(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	# 	print('that come from the WTC Painleve analysis of the Kawachara equation ....')
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	phi, f, U4 = symbols('phi f U4', cls = Function)
	# 	Kawachara_PDE = parse_expr('diff(f(x, t), t)+f(x, t)*diff(f(x, t), x)+diff(f(x, t), x, x, x)+diff(f(x, t), x, x, x, x, x)')
	# 	# print(integrate(Kawachara_PDE.subs(f(x, t), diff(f(x, t), x)), x))
	# 	alpha, transform, compatibility_equations = ultimatePainleve(
	# 			Kawachara_PDE, # Input PDE
	# 			x, # The input variable used for the spatial variable
	# 			t # The input variable used for the temporal variable
	# 			# alpha # The power balancing exponent
	# 		)
	# 	U4soln = solve(compatibility_equations[0], U4(x, t))[0]
	# 	transform_new = transform.subs(U4(x, t), U4soln)
	# 	KawacharaPDE_eqn = Kawachara_PDE.subs(f(x, t), transform_new)
	# 	KawacharaPDE_eqn, _ = fraction(together(KawacharaPDE_eqn.doit()))
	# 	KawacharaPDE_eqn = expand(KawacharaPDE_eqn)
	# 	for k in range(len(compatibility_equations)):
	# 		compatibility_equations[k] = compatibility_equations[k].subs(U4(x, t), U4soln)
	# 		compatibility_equations[k], _ = fraction(together(compatibility_equations[k].doit()))
	# 		compatibility_equations[k] = expand(compatibility_equations[k])
	# 	# self.assertTrue(True)
	# 	compatibility_equations = [compatibility_equations[k] for k in range(1, len(compatibility_equations))]
	# 	compatibility_equations.insert(0, KawacharaPDE_eqn)
	# 	for k in range(len(compatibility_equations)):
	# 		Factors = factor_list(compatibility_equations[k])
	# 		compatibility_equations[k] = Factors[1][-1][0]
	# 		print('------------------------------')
	...
	# 	status = compatibility_checker[1]
	# 	if status:
	# 		print('The PDE system from WTC Painleve analysis of the Kawachara equation renders compatible')
	# 	else:
	# 		print('The PDE system from WTC Painleve analysis of the Kawachara equation renders incompatible')
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	self.assertFalse(status)

	# def test_TestCase8_FisherKPP(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	...
	# 	U2soln = solve(compatibility_equations[0], U2(x, t))[0]
	# 	transform_new = transform.subs(U2(x, t), U2soln)
	# 	FisherKPP_eqn = FisherKPP_PDE.subs(f(x, t), transform_new)
	...
	# def test_TestCase9_FitzHughNagumo(self):
	# 	print('------------------------------------------------------------------------------------------------------------------------------------')
	# 	print('Here I am testing for the compatibility of the system of PDEs')
	...
	# 	U1soln = solve(compatibility_equations[0], U1(x, t))[0]
	# 	transform_new = transform.subs(U1(x, t), U1soln)
	# 	FitzHughNagumoPDE_eqn = FitzHughNagumo_PDE.subs(f(x, t), transform_new)
