...
KdVBurgers_Equation = parse_expr('diff(f(x,t),t)+f(x,t)*diff(f(x,t),x)-sigma*diff(f(x,t),x,x)+kappa*diff(f(x,t),x,x,x)')
result_package = ultimatePainleve(KdVBurgers_Equation, x, t)
print('------------------------------------------------------------------')
print('Final answer of the sandbox program with KdV-Burgers equation')
print('------------------------------------------------------------------')
...
if context_free_compatible_store[1]:
	...
	if ode_order(KdVBurgers_Equation, f) > 2:
		context_free_compatible_store = ContextFreeDifferentialCommutationCompatibilitySystemPDEs(result_package[2][k], u_max)
		if context_free_compatible_store[1]:
			flag_switch = True
			flag_on_umax = True
...
if not(flag_switch):
	...
	if ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(list(set(result_package[2][k])-set([result_package[2][k][-1],result_package[2][k][-2]])), [result_package[2][k][-2]], phi, [u_max]):
		print('The system of PDEs is context-sensitive compatible and KdV-Burgers equation is integrable')
	else:
		print('The system of PDEs is context-sensitive incompatible and KdV-Burgers equation is non-integrable')
else:
	print('The system of PDEs is context-free compatible as 0 belongs to g_{ '+str(context_free_compatible_store[0]+1)+'}')
	if ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(list(set(result_package[2][k])-set([result_package[2][k][-1],result_package[2][k][-2]])), [result_package[2][k][-1]], phi, [u_max]):
		print('The system of PDEs is context-sensitive compatible and KdV-Burgers equation is integrable')
	else:
		print('The system of PDEs is not context-sensitive compatible and KdV-Burgers equation is non-integrable')
