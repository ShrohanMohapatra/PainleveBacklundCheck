...
function_PDE = parse_expr('diff(f(x, t), t)+diff(f(x, t), x)+f(x, t)*diff(f(x, t), x)-diff(f(x, t), x, x, t)')
...
for k in range(len(totalTable)):
	print('--------------------------------------------')
	print('--------------------------------------------')
	compatibility_Conditions[k], _ = fraction(together(compatibility_Conditions[k].doit()))
