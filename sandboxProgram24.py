...
class PainleveBacklundCheckApp(MDApp):
	...
	def rearranger_function(self, S_variables, solution_dictionary_for_all_higher_derivs):
		reference_variable = S_variables[0]
		for variable in solution_dictionary_for_all_higher_derivs:
			solution = solution_dictionary_for_all_higher_derivs[variable]
			expression_terms = solution.as_coefficients_dict()
			...
			print('solution in the loop -> ', solution)
			print('list_of_differential_terms ->', list_of_differential_terms)
			for differential_term in list_of_differential_terms:
				max_x_derivative, max_t_derivative = 5, 5
				x_deriv = 0
				t_deriv = 0
				for other_variable in solution_dictionary_for_all_higher_derivs:
					if other_variable != variable:
						for k in range(max_x_derivative):
							for m in range(max_t_derivative):
								if k == 0 and m == 0:
									continue
								higher_differential_term_of_the_other_variable = expand(diff(other_variable, (x, k), (t, m)))
								print(
									'k -> ', k, ' m -> ', m,
									'differential_term -> ', differential_term,
									'other_variable -> ', other_variable,
									'higher_differential_term_of_the_other_variable -> ',
									higher_differential_term_of_the_other_variable,
									'differential_term == higher_differential_term_of_the_other_variable -> ',
									differential_term == higher_differential_term_of_the_other_variable
									)
								if differential_term == higher_differential_term_of_the_other_variable:
									x_deriv, t_deriv = k, m
									reference_variable = other_variable
									return [x_deriv, t_deriv, reference_variable]
		return [x_deriv, t_deriv, reference_variable]
	...
	def ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(self, systemsOfPDEs, contextualPDEs, phi, list_of_u_functions):
		...
		while True:
			[x_deriv, t_deriv, reference_variable] = self.rearranger_function(S_variables, solution_dictionary_for_all_higher_derivs)

			if reference_variable == S_variables[0] and x_deriv == 0 and t_deriv == 0:
				break
			else:
				print('New variable loop taken -> x_deriv = ', x_deriv, ' t_deriv = ', t_deriv)
				if x_deriv == 0 and t_deriv == 0:
					equation = expand(variable-solution_dictionary_for_all_higher_derivs[variable]).doit()
					other_equation = expand(reference_variable-solution_dictionary_for_all_higher_derivs[reference_variable]).doit()
					S_sub_equation = [equation, other_equation]
					S_sub_variables = [variable, reference_variable]
					solution_to_data_structure = list(nonlinsolve(S_sub_equation, S_sub_variables))
					solution_extractor = solution_to_data_structure[0]
					solution_dictionary_for_all_higher_derivs[variable] = solution_extractor[0]
					solution_dictionary_for_all_higher_derivs[reference_variable] = solution_extractor[1]
					S_equations[S_variables.index(variable)] = solution_extractor[0]
					S_equations[S_variables.index(reference_variable)] = solution_extractor[1]
				else:
					equation = expand(variable-solution_dictionary_for_all_higher_derivs[variable]).doit()
					equation_for_higher_deriv = expand(diff(reference_variable,(x,x_deriv),(t,t_deriv))- diff(solution_dictionary_for_all_higher_derivs[reference_variable],(x,x_deriv),(t,t_deriv))).doit()
					equation_for_other_variable = expand(reference_variable - solution_dictionary_for_all_higher_derivs[reference_variable]).doit()
					S_sub_equation = [equation, equation_for_higher_deriv, equation_for_other_variable]
					S_sub_variables = [variable, diff(reference_variable,(x,x_deriv),(t,t_deriv)), reference_variable]
					solution_to_data_structure = list(nonlinsolve(S_sub_equation, S_sub_variables))
					solution_extractor = solution_to_data_structure[0]
					solution_dictionary_for_all_higher_derivs[variable] = solution_extractor[0]
					solution_dictionary_for_all_higher_derivs[diff(reference_variable,(x,x_deriv),(t,t_deriv))] = solution_extractor[1]
					solution_dictionary_for_all_higher_derivs[reference_variable] = solution_extractor[2]
					S_equations[S_variables.index(variable)] = expand(variable - solution_extractor[0])
					S_equations[S_variables.index(reference_variable)] = expand(reference_variable - solution_extractor[2])
					S_equations.append(expand(diff(reference_variable,(x,x_deriv),(t,t_deriv)) - solution_extractor[1]))
					S_variables.append(diff(reference_variable,(x,x_deriv),(t,t_deriv)))
		...
		for variable in solution_dictionary_for_all_higher_derivs:
			print('solution_dictionary_for_all_higher_derivs[',variable,'] = ',solution_dictionary_for_all_higher_derivs[variable])
		# We need fresh highest orders of phi in the list_of_all_highest_derivative_terms_in_phi
		for variable in solution_dictionary_for_all_higher_derivs:
			temp_variable = variable
			temp_variable_2 = variable
			if isinstance(temp_variable, Derivative):
				list_of_partial_derivatives = list(temp_variable.variables)
				for inner_variable in list_of_partial_derivatives:
					temp_variable = integrate(temp_variable, inner_variable)
			if temp_variable == phi and temp_variable_2 not in list_of_all_highest_derivative_terms_in_phi:
				list_of_all_highest_derivative_terms_in_phi.append(temp_variable_2)
			if temp_variable == phi and temp_variable_2 not in S_variables:
				S_variables.append(temp_variable_2)
		print('list_of_all_highest_derivative_terms_in_phi = ', list_of_all_highest_derivative_terms_in_phi)
		...
		for w in range(len(list_of_A_orders)):
			...
			for derivsInX in range(A_max-Aw+2):
				print('derivsInX = ', derivsInX)
				for derivsInT in range(B_max-Bw+2):
					if derivsInX == 0 and derivsInT == 0: continue
					...
					for other_variable in solution_dictionary_for_all_higher_derivs:
						...
						if other_variable != variable:
							equation = equation.subs(other_variable, solution_dictionary_for_all_higher_derivs[other_variable]).doit()
							equation = expand(equation)
		...
		while True:
			...
			for variable in solution_dictionary_for_all_higher_derivs:
				solution = solution_dictionary_for_all_higher_derivs[variable]
				print('variable -> ', variable, ' solution -> ', solution)
				solution_temp = solution
				for other_variable in solution_dictionary_for_all_higher_derivs:
					if other_variable != variable:
						solution = expand(solution.subs(other_variable, solution_dictionary_for_all_higher_derivs[other_variable]).doit())
						solution = expand(solution)
				difference_in_the_solution = expand(solution-solution_temp)
				flag = flag and difference_in_the_solution == 0
				if difference_in_the_solution != 0:
					solution_dictionary_for_all_higher_derivs[variable] = solution
			counter = counter + 1
			if flag: break
		S_equations = []
		...
		while True:
			...
			solution_to_data_structure = solve(S_equations, S_variables)
	...
	def BacklundContextFreeAndContextSensitiveCompatibility(self):
		...
		if flag_context_free_compatibility[1]:
			...
			while len(filteredSystemOfPDEs)+len(contextualPDEs) >= 2:
				switch_track = False
				for k in range(len(filteredSystemOfPDEs)-1,-1,-1):
					f1 = filteredSystemOfPDEs[k]
					print('f1 = ', f1)
