...
KdV_Equation = parse_expr('diff(f(x,t),t)+f(x,t)*diff(f(x,t),x)+sigma*diff(f(x,t),(x,3))')
result_package = ultimatePainleve(KdV_Equation, x, t)
print('------------------------------------------------------------------')
...
if not(flag_switch):
	...
	print('System of PDEs -> '+str(result_package[2][k]))
	if flag_on_phi: print('It is based on:', str(phi))
	if flag_on_umax: print('It is based on:', str(u_max))

systemsOfPDEs = list(set(result_package[2][k])-set([result_package[2][k][-1],result_package[2][k][-2]]))
contextualPDEs = [result_package[2][k][-1]]
list_of_u_functions = [u_max]
if ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(systemsOfPDEs, contextualPDEs, phi, list_of_u_functions):
	print('KdV equation is integrable')
else: print('KdV equation is non-integrable')
