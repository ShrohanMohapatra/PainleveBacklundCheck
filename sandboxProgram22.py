...
class PainleveBacklundCheckApp(MDApp):
	...
	def ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(self, systemsOfPDEs, contextualPDEs, phi, list_of_u_functions):
		...
		for variable in solution_dictionary_for_all_higher_derivs:
			...
			stop_detector = [False, False, False]
						...
						for k in range(max_x_derivative):
							for m in range(max_t_derivative):
								...
								if differential_term == higher_differential_term_of_the_other_variable:
									...
									stop_detector[0] = True
									break
							if stop_detector[0]:
								stop_detector[1] = True
								break
						if stop_detector[1]:
							stop_detector[2] = True
							break
				if stop_detector[2]: break
		if (reference_variable == variable or reference_variable == S_variables[0]) and x_deriv == 0 and t_deriv == 0: pass
		else:
		...
		for v in range(len(list_of_all_highest_derivative_terms_in_uk)):
			...
			for i in range(1, A_max-A_min+2):
				for j in range(1, B_max-B_min+2):
		...
		for w in range(len(list_of_A_orders)):
			...
			for derivsInX in range(1, A_max-Aw+2):
				print('derivsInX = ', derivsInX)
				for derivsInT in range(1, B_max-Bw+2):
