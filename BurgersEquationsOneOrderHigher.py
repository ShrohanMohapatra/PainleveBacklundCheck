...
T1 = Equation3_compatibility_test
T2 = Equation4_compatibility_test

print('--------------------------------------------------------------------------')
print('T1 = ', showDiffNotation(T1))
print('--------------------------------------------------------------------------')
print('--------------------------------------------------------------------------')
print('T2 = ', showDiffNotation(T2))
print('--------------------------------------------------------------------------')

partialT_1_partialt = diff(T1, t)
partialT_2_partialx = diff(T2, x)

print('--------------------------------------------------------------------------')
print('partialT_1_partialt = ', showDiffNotation(partialT_1_partialt))
print('--------------------------------------------------------------------------')
print('--------------------------------------------------------------------------')
print('partialT_2_partialx = ', showDiffNotation(partialT_2_partialx))
print('--------------------------------------------------------------------------')

partialT_1_partialt_plus_2sigma_phixsq_partialT_2_partialx = expand(diff(T1, t) + 2*sigma*diff(phi,x)**2*diff(T2, x))

print('--------------------------------------------------------------------------')
print('diff(T1, t) + 2*sigma*diff(phi,x)^2*diff(T2, x) = ',
      showDiffNotation(partialT_1_partialt_plus_2sigma_phixsq_partialT_2_partialx))
print('--------------------------------------------------------------------------')

u2t_eliminant = expand(diff(T1, t) + 2*sigma*diff(phi,x)**2*diff(T2, x) - 2*sigma*diff(phi,x)*diff(phi,x,x)*T2)
print('--------------------------------------------------------------------------')
print('diff(T1, t) + 2*sigma*diff(phi,x)**2*diff(T2, x) - 2*sigma*diff(phi,x)*diff(phi,x,x)*T2 =', showDiffNotation(u2t_eliminant))
print('--------------------------------------------------------------------------')

u2x_extractor = expand(
		 (diff(T1, t) + 2*sigma*diff(phi,x)**2*diff(T2, x) - 2*sigma*diff(phi,x)*diff(phi,x,x)*T2)*(-diff(phi,t,t)*diff(phi,x)**2 + 2*diff(phi,t,x,x)*diff(phi,x)**2*sigma + 2*diff(phi,t,x)*diff(phi,t)*diff(phi,x) - 4*diff(phi,t,x)*diff(phi,x,x)*diff(phi,x)*sigma - diff(phi,t)**2*diff(phi,x,x) - 2*diff(phi,t)*diff(phi,x,x,x)*diff(phi,x)*sigma + 4*diff(phi,t)*diff(phi,x,x)**2*sigma - diff(phi,x,x,x,x)*diff(phi,x)**2*sigma**2 + 4*diff(phi,x,x,x)*diff(phi,x,x)*diff(phi,x)*sigma**2 - 3*diff(phi,x,x)**3*sigma**2 - 2*diff(phi,x,x)*diff(phi,x)**3*sigma*u2) - \
		 T1*(-diff(phi,t,t,t)*diff(phi,x)**2 + 2*diff(phi,t,t,x,x)*diff(phi,x)**2*sigma + 2*diff(phi,t,t,x)*diff(phi,t)*diff(phi,x) - 4*diff(phi,t,t,x)*diff(phi,x,x)*diff(phi,x)*sigma - 2*diff(phi,t,t)*diff(phi,t)*diff(phi,x,x) - 2*diff(phi,t,t)*diff(phi,x,x,x)*diff(phi,x)*sigma + 4*diff(phi,t,t)*diff(phi,x,x)**2*sigma - diff(phi,t,x,x,x,x)*diff(phi,x)**2*sigma**2 - 2*diff(phi,t,x,x,x)*diff(phi,t)*diff(phi,x)*sigma + 4*diff(phi,t,x,x,x)*diff(phi,x,x)*diff(phi,x)*sigma**2 - diff(phi,t,x,x)*diff(phi,t)**2 + 8*diff(phi,t,x,x)*diff(phi,t)*diff(phi,x,x)*sigma + 4*diff(phi,t,x,x)*diff(phi,x,x,x)*diff(phi,x)*sigma**2 - 9*diff(phi,t,x,x)*diff(phi,x,x)**2*sigma**2 - 4*diff(phi,t,x,x)*diff(phi,x)**3*sigma*u2 + 2*diff(phi,t,x)**2*diff(phi,t) - 4*diff(phi,t,x)**2*diff(phi,x,x)*sigma - 2*diff(phi,t,x)*diff(phi,t)*diff(phi,x,x,x)*sigma - 2*diff(phi,t,x)*diff(phi,x,x,x,x)*diff(phi,x)*sigma**2 + 4*diff(phi,t,x)*diff(phi,x,x,x)*diff(phi,x,x)*sigma**2 - 4*diff(phi,t,x)*diff(phi,x,x)*diff(phi,x)**2*sigma*u2 + 2*diff(phi,t)*diff(phi,x,x,x)*diff(phi,x)**2*sigma*u2 - 2*diff(phi,t)*diff(phi,x,x)**2*diff(phi,x)*sigma*u2 + 2*diff(phi,x,x,x,x)*diff(phi,x)**3*sigma**2*u2 - 4*diff(phi,x,x,x)*diff(phi,x,x)*diff(phi,x)**2*sigma**2*u2 + 2*diff(phi,x,x)**3*diff(phi,x)*sigma**2*u2 + 4*diff(phi,x,x)*diff(phi,x)**4*sigma*u2**2)
		 )
generator_list = [diff(u2, x), diff(u2, x, x), diff(u2, x, x, x)]
u2x_extractor = poly(u2x_extractor, generator_list)
print('--------------------------------------------------------------------------')
print('(diff(T1, t) + 2*sigma*diff(phi,x)**2*diff(T2, x) - 2*sigma*diff(phi,x)*diff(phi,x,x)*T2)'+\
      '*(-diff(phi,t,t)*diff(phi,x)**2 + 2*diff(phi,t,x,x)*diff(phi,x)**2*sigma + '+\
			'2*diff(phi,t,x)*diff(phi,t)*diff(phi,x) - 4*diff(phi,t,x)*diff(phi,x,x)*diff(phi,x)*sigma '+\
			'- diff(phi,t)**2*diff(phi,x,x) - 2*diff(phi,t)*diff(phi,x,x,x)*diff(phi,x)*sigma + '+\
			'4*diff(phi,t)*diff(phi,xx)**2*sigma - diff(phi,x,x,x,x)*diff(phi,x)**2*sigma**2 + '+\
			'4*diff(phi,x,x,x)*diff(phi,x,x)*diff(phi,x)*sigma**2 - 3*diff(phi,x,x)**3*sigma**2 -'+\
			'2*diff(phi,x,x)*diff(phi,x)**3*sigma*u2) - '+\
      'T1*(-diff(phi,t,t,t)*diff(phi,x)**2 + 2*diff(phi,t,t,x,x)*diff(phi,x)**2*sigma '+\
			'+ 2*diff(phi,t,t,x)*diff(phi,t)*diff(phi,x) - 4*diff(phi,t,t,x)*diff(phi,x,x)*diff(phi,x)*sigma -'+\
			' 2*diff(phi,t,t)*diff(phi,t)*diff(phi,x,x) - 2*diff(phi,t,t)*diff(phi,x,x,x)*diff(phi,x)*sigma + '+\
			'4*diff(phi,t,t)*diff(phi,x,x)**2*sigma - diff(phi,t,x,x,x,x)*diff(phi,x)**2*sigma**2 - '+\
			'2*diff(phi,t,x,x,x)*diff(phi,t)*diff(phi,x)*sigma + 4*diff(phi,t,x,x,x)*diff(phi,x,x)*diff(phi,x)*sigma**2 - '+\
			'diff(phi,t,x,x)*diff(phi,t)**2 + 8*diff(phi,t,x,x)*diff(phi,t)*diff(phi,x,x)*sigma + '+\
			'4*diff(phi,t,x,x)*diff(phi,x,x,x)*diff(phi,x)*sigma**2 - 9*diff(phi,t,x,x)*diff(phi,x,x)**2*sigma**2 - '+\
			'4*diff(phi,t,x,x)*diff(phi,x)**3*sigma*u2 + 2*diff(phi,t,x)**2*diff(phi,t) - '+\
			'4*diff(phi,t,x)**2*diff(phi,x,x)*sigma - 2*diff(phi,t,x)*diff(phi,t)*diff(phi,x,x,x)*sigma - '+\
			'2*diff(phi,t,x)*diff(phi,x,x,x,x)*diff(phi,x)*sigma**2 + '+\
			'4*diff(phi,t,x)*diff(phi,x,x,x)*diff(phi,x,x)*sigma**2 - '+\
			'4*diff(phi,t,x)*diff(phi,x,x)*diff(phi,x)**2*sigma*u2 + '+\
			'2*diff(phi,t)*diff(phi,x,x,x)*diff(phi,x)**2*sigma*u2 - '+\
			'2*diff(phi,t)*diff(phi,x,x)**2*diff(phi,x)*sigma*u2 + 2*diff(phi,x,x,x,x)*diff(phi,x)**3*sigma**2*u2 - '+\
			'4*diff(phi,x,x,x)*diff(phi,x,x)*diff(phi,x)**2*sigma**2*u2 + '+\
			'2*diff(phi,x,x)**3*diff(phi,x)*sigma**2*u2 + 4*diff(phi,x,x)*diff(phi,x)**4*sigma*u2**2) = \n',
			showDiffNotation(u2x_extractor))
print('--------------------------------------------------------------------------')
