...
class PainleveBacklundCheckApp(MDApp):
	...
	def ContextFreeDifferentialCommutationCompatibilitySystemPDEs(self, systemsOfPDEs, func, depth_max = 150, depth = 0):
		...
		for k in range(len(systemsOfPDEs)):
			...
			Clock.schedule_once(self.scheduleOnceTester, 0)
			try:
				self.string_storer = str(showDiffNotation(systemsOfPDEs[k]))
				Clock.schedule_once(self.scheduleOnceTester, 0)
			except:
				raise
		...
		if len(systemsOfPDEs) == 2:
			...
			for k in range(len(systemsOfPDEs_F)):
				...
				Clock.schedule_once(self.scheduleOnceTester, 0)
				self.string_storer = str(showDiffNotation(systemsOfPDEs_F[k]))
				Clock.schedule_once(self.scheduleOnceTester, 0)
	...
	def ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(self, systemsOfPDEs, contextualPDEs, phi, list_of_u_functions):
		...
		for k in range(n):
			...
			self.string_storer = str(showDiffNotation(solution_dictionary_for_all_higher_derivs))
			Clock.schedule_once(self.scheduleOnceTester, 0)
		...
		flag2 = True
		phi_term_random = list_of_all_highest_derivative_terms_in_phi.choice()
		for k in range(n-1):
	...
	def calculate_compatibility(self):
		...
		if self.root.ids.input_pde.text == 'diff(f(x,t),t)+f(x,t)*diff(f(x,t),x)+sigma*diff(f(x,t),(x,3))':
			...
			for term in dictionary_of_solutions:
				self.string_storer = str(showDiffNotation(dictionary_of_solutions[term]))
				Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			result_package = self.ultimatePainleve(self.root.ids.input_pde.text, x, t)
			umber_Of_Sets = len(result_package[2])
			alpha = result_package[0]
			self.backlund_transform = result_package[1]
			u_max = Function('U'+str(-alpha))(x, t)
			...
			self.string_storer = str(CompatibilitySystemPDEs([
					Equation0,
					Equation1Storage,
					Equation2Storage,
			...
			eqn51 = Equation1
			eqn54 = Equation3
			self.string_storer = str(showDiffNotation(eqn51))+'= 0'
			Clock.schedule_once(self.scheduleOnceTester, 0)
			self.string_storer = str(showDiffNotation(eqn54))+'= 0'
			Clock.schedule_once(self.scheduleOnceTester, 0)
			solution = solve([eqn51,eqn54],[diff(phi,t,t),diff(phi,t,x,x,x)])

			self.string_storer = str(showDiffNotation(solution[diff(phi,t,t)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			self.string_storer = str(showDiffNotation(solution[diff(phi,t,x,x,x)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			eqn55 = Equation4
			eqn56 = Equation5
			
			self.string_storer = str(showDiffNotation(eqn55))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			self.string_storer = str(showDiffNotation(eqn56))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			u1_txx_solution = solve(eqn55,diff(u1,t,x,x))[0]
			u2_txx_solution = solve(eqn56,diff(u2,t,x,x))[0]

			self.string_storer = str(showDiffNotation(u1_txx_solution))
			Clock.schedule_once(self.scheduleOnceTester, 0)

			self.string_storer = str(showDiffNotation(u2_txx_solution))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			self.string_storer = str(showDiffNotation(solution_list[diff(phi,t,t,x,x)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			self.string_storer = str(showDiffNotation(solution_list[diff(phi,t,x,x,x)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			self.string_storer = str(showDiffNotation(equation1))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			self.string_storer = str(showDiffNotation(equation2))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			compatibility_check = equation2 - equation1
			compatibility_check = expand(compatibility_check)
			self.string_storer = str(showDiffNotation(compatibility_check))
			self.compatibility_test = int(compatibility_check == 0)
			Clock.schedule_once(self.scheduleOnceTester, 0)

		elif self.root.ids.input_pde.text == 'diff(f(x,t),t)+f(x,t)*diff(f(x,t),x)-sigma*diff(f(x,t),x,x)+kappa*diff(f(x,t),(x,3))':
			f = Function('f')
			u0 = Function('u0')(x, t)
			u1 = Function('u1')(x, t)
			u2 = Function('u2')(x, t)
			...
			u0_solution = solve(Equation0, u0)[0]
			Equation0_residual = Equation0.subs(u0, u0_solution)
			self.string_storer = str(showDiffNotation(u0_solution))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			Equation1_residual, _ = fraction(together(Equation1_residual).doit())
			Equation1_residual = expand(Equation1_residual)
			self.string_storer = str(showDiffNotation(u1_solution))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			self.string_storer = str(showDiffNotation(u2_solution))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			factors_list_of_equation3 = factor_list(Equation3)
			Equation3 = factors_list_of_equation3[-1][-1][0]
			self.string_storer = str(showDiffNotation(Equation3_compatibility_test))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			factors_list_of_equation4 = factor_list(Equation4)
			Equation4 = factors_list_of_equation4[-1][-1][0]
			self.string_storer = str(showDiffNotation(Equation4_compatibility_test))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			self.string_storer = str(showDiffNotation(Equation5_compatibility_test))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			self.string_storer = str(showDiffNotation(CompatibilitySystemPDEs(
				[
					Equation2,
					Equation3_compatibility_test,
					Equation4_compatibility_test,
			...
			self.string_storer = str(showDiffNotation(dictionary_of_solutions[diff(phi, t)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			self.string_storer = str(showDiffNotation(dictionary_of_solutions[diff(phi, x, t)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)

			self.string_storer = str(showDiffNotation(dictionary_of_solutions[diff(phi, (x, 4))]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			
			self.string_storer = str(showDiffNotation(dictionary_of_solutions[diff(u2, t)]))
			Clock.schedule_once(self.scheduleOnceTester, 0)
			...
			for k in range(5,max_number_of_derivatives):
				...
				assert ode_order(numerator, phi) == 3
				equation_new = together(numerator/denominator).doit()
				self.string_storer = str(showDiffNotation(equation_new))
				Clock.schedule_once(self.scheduleOnceTester, 0)
				dictionary_of_solutions[diff(phi,(x,k))] = equation_new
			...
			for k in range(1, max_number_of_derivatives):
				...
				print('----------------------------------------------------------------------')
				print_latex(showDiffNotation(equation_new))
				print('----------------------------------------------------------------------')
				self.string_storer = str(showDiffNotation(equation_new))
				Clock.schedule_once(self.scheduleOnceTester, 0)
				dictionary_of_solutions[diff(u2,(x,k),t)] = equation_new
			...
			while True:
				...
				numerator = expand(numerator)
				order_param = ode_order(numerator, phi)
				self.string_storer = str(showDiffNotation(order_param))
				Clock.schedule_once(self.scheduleOnceTester, 0)
				if order_param < 4: break
