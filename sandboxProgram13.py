...
if len([phi, u_max]) == len(list(set(result_package[2][k])-set([result_package[2][k][1]]))):
	if flag_switch:
		print('The system of PDEs ->'+str(result_package[2][k])+'is perfectly determinate and thus Burgers PDE is integrable.')
else:
	if ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs([result_package[2][k][0]], [result_package[2][k][1]], phi, [u_max]):
		print('Burgers equation is integrable')
	else: print('Burgers equation is non-integrable')
