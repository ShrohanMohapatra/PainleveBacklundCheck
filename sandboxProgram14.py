...
sineGordon_Equation = parse_expr('2*f(x,t)*diff(f(x,t),x,t)-2*diff(f(x,t),x)*diff(f(x,t),t)-f(x,t)**3+f(x,t)')
result_package = ultimatePainleve(sineGordon_Equation, x, t)
print('------------------------------------------------------------------')
print('Final answer of the sandbox program with sine-Gordon equation')
print('------------------------------------------------------------------')
...
if context_free_compatible_store[1]:
	...
	if ode_order(sineGordon_Equation, f) > 2:
		context_free_compatible_store = ContextFreeDifferentialCommutationCompatibilitySystemPDEs(result_package[2][k], u_max)
		if context_free_compatible_store[1]:
			flag_switch = True
			flag_on_umax = True
...
if not(flag_switch):
	...
	if ContextSensitiveDifferentialCommutationCompatibilitySystemPDEs(list(set(result_package[2][k])-set([result_package[2][k][-1]])), [result_package[2][k][-1]], phi, [u_max]):
		print('The system of PDEs is context-sensitive compatible and sine-Gordon equation is integrable')
	else:
		print('The system of PDEs is context-sensitive incompatible and sine-Gordon equation is non-integrable')
else:
